...
async function initializeParser() {
  try {
    ...
    if (url.includes("zhipin.com")) {
      ...
      const { BossParser } = await import(
        extensionUrl + "content_scripts/sites/boss.js"
      );
      ...
      ParserName = "lagou";
      const { LagouParser } = await import(
        extensionUrl + "content_scripts/sites/lagou.js"
      );
      ...
      ParserName = "liepin";
      const { LiepinParser } = await import(
        extensionUrl + "content_scripts/sites/liepin.js"
      );
      ...
      ParserName = "hliepin";
      const { HLiepinParser } = await import(
        extensionUrl + "content_scripts/sites/hliepin.js"
      );
      ...
      const { Employer58Parser } = await import(
        extensionUrl + "content_scripts/sites/employer58.js"
      );
      ...
      ParserName = "zhilian";
      const { ZhilianParser } = await import(
        extensionUrl + "content_scripts/sites/zhilian.js"
      );
...
function randomDelay(message2 = "无") {
  ...
  const delaySeconds = Math.floor(
    Math.random() * (actualMax - actualMin + 1) + actualMin,
  );
...
async function startAutoScroll() {
  ...
  if (currentParser?.aiMode) {
    ...
    if (
      ...
      !currentParser.aiSettings.jobDescription.trim()
    ) {
  ...
  try {
    ...
    for (const doc of documents) {
      ...
      const unprocessedElements = elements.filter((el) => {
        ...
        const absoluteTop =
          rect.top +
          (doc === document ? window.pageYOffset : doc.defaultView.pageYOffset);
      ...
      if (unprocessedElements.length > 0) {
        ...
        try {
          ...
          const tempClass =
            "temp-highlight-" + Math.random().toString(36).substr(2, 9);
        ...
        if (
          ParserName === "hliepin" &&
          currentParser &&
          typeof currentParser.shouldNavigateToNextPage === "function"
        ) {
          const shouldNavigate =
            currentParser.shouldNavigateToNextPage(elements);
...
async function processElement(element, doc) {
  if (
    currentParser.aiSettings == null ||
    currentParser.aiSettings.communicationConfig == null
  ) {
  ...
  try {
    // 发送计数请求到服务器（不等待响应）
    const apiBase = window.GOODHR_CONFIG
    ...
    if (!targetElement) {
      targetElement = element.closest(
        `[class*="${currentParser.selectors.items}"]`,
      );
    }
        ...
        if (clickCandidate) {
          ...
          if (clicked) {
            ...
            if (currentParser.aiMode) {
              ...
              shouldContact =
                currentParser.filterCandidate(simpleCandidateInfo);
              if (ParserName === "employer58") {
                shouldContact = true;
              }
            }
          ...
          if (clicked) {
            try {
              if (
                ...
                const phone = await currentParser.collectPhoneWechatResume(
                  currentParser.aiSettings.communicationConfig.collectPhone,
                  currentParser.aiSettings.communicationConfig.collectWechat,
                  currentParser.aiSettings.communicationConfig.collectResume,
                  candidate,
    ...
    switch (message.action) {
      case "START_SCROLL":
        ...
        if (message.data.clickFrequency !== undefined) {
          currentParser.clickCandidateConfig.frequency =
            message.data.clickFrequency;
        }
...
function removeAds() {
  try {
    ...
    const adElements = document.querySelectorAll(
      ".ad-container, .draggable-ad-container",
    );
...
function createDraggableAdElement(adData) {
  ...
  function dragEnd(e) {
    ...
    if (dragDuration < 200) {
      const moveDistance = Math.sqrt(
        Math.pow(xOffset, 2) + Math.pow(yOffset, 2),
      );
      if (moveDistance < 5) {
  ...
  if (adData.link) {
    adElement.addEventListener("click", (e) => {
      ...
      // 不处理关闭按钮和标题栏的点击
      if (
        e.target.classList.contains("ad-close-btn") ||
        e.target.classList.contains("ad-header") ||
        (e.target.parentElement &&
          e.target.parentElement.classList.contains("ad-header"))
      ) {
        e.preventDefault();
        e.stopPropagation();
        return;
...
function displayAds(isAIExpired) {
  ...
  const targetElements = document.querySelectorAll(
    '[class*="list"], [class*="container"], [class*="content"]',
  );
