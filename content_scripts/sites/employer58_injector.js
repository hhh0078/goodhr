// 58同城拦截器注入器 - 在document_start时运行
(function injectEmployer58Interceptor() {
  "use strict";
...
// console.log('✅ 58同城拦截器注入器已加载');
  ...
  window.addEventListener("load", async function () {
    ...
    if (isCandidateDetailPage()) {
      ...
      // 检查时间戳，判断是否是插件自动点击
      const clickTimestamp = localStorage.getItem(
  ...
  async function extractCandidateDetail() {
    try {
      ...
      while (attempts < maxAttempts) {
        ...
        // 获取文本内容
        textElement = document.querySelector(
      ...
      if (
        ...
        (!candidate.name || !candidate.textContent)
      ) {
