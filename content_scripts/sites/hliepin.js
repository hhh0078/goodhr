...
class HLiepinParser extends BaseParser {
  constructor() {
    ...
    this.selectors = {
      ...
      confirmClickTarget:
        "ant-btn ant-btn-link ant-btn-lg btn-cancel directly-open-chat-btn",
      ...
      phoneButton: [
        "ant-btn ant-btn-primary __im_basic__basic-input-action",
        "im-ui-action-button action-item",
      ],
    };
  ...
  async collectPhoneWechatResume(phone, wechat, resume, candidate, element) {
    try {
      ...
      for (let i = 0; i < this.selectors.phoneButton.length; i++) {
        //先点击继续沟通
        const continueButton = await this.CommonGetElementsByClassName(
          element,
          this.selectors.phoneButton[i],
        );
      ...
      if (phone) {
        ...
        if (phoneButton) {
          ...
          const confirmButton = await this.CommonQuerySelector(
            document,
            `[class^="ant-im-modal-confirm-btns"]`,
          );
          if (confirmButton) {
            const confirmButton1 = this.CommonQuerySelectorAll(
              confirmButton,
              `[class^="ant-im-btn ant-im-btn-primary"]`,
            );
  ...
  isAtBottomOfPage() {
    ...
    // 检查是否有加载更多或无更多数据的提示
    const hasNoMoreData = document.querySelector(
      '[class*="no-more"], [class*="no-data"], [class*="end"]',
    );
    ...
    console.log(
      `页面滚动位置检查: scrollTop=${scrollTop}, windowHeight=${windowHeight}, documentHeight=${documentHeight}`,
    );
    console.log(
      `是否接近底部: ${isNearBottom}, 是否无更多数据: ${!!hasNoMoreData}, 是否正在加载: ${!!hasLoadingIndicator}`,
    );
  ...
  hasNextPageButton() {
    const nextPageButton = document.querySelector(
      `[class*="${this.selectors.nextPageButton}"]`,
    );
    if (!nextPageButton) {
      return false;
    }
  ...
  async clickNextPageButton() {
    try {
      ...
      if (
        nextPageButton &&
        !nextPageButton.classList.contains(this.selectors.disabledClass)
      ) {
  ...
  findElementsByMultipleMethods(selector, options = {}) {
    ...
    if (
      (methods.includes("all") || methods.includes("getElementsByClassName")) &&
      selector.startsWith(".")
    ) {
      try {
        ...
        if (filteredElements.length > 0) {
          console.log(
            `方法2 (getElementsByClassName) 找到 ${filteredElements.length} 个元素`,
          );
          return {
    ...
    if (
      (methods.includes("all") || methods.includes("getElementsByTagName")) &&
      ...
      try {
        ...
        if (filteredElements.length > 0) {
          console.log(
            `方法3 (getElementsByTagName) 找到 ${filteredElements.length} 个元素`,
          );
    ...
    if (methods.includes("all") || methods.includes("textSearch")) {
      try {
        ...
        if (keywords.length > 0) {
          ...
          if (filteredElements.length > 0) {
            console.log(
              `方法5 (文本搜索) 找到 ${filteredElements.length} 个元素`,
            );
    ...
    try {
      if (elements) {
        ...
        items = document.querySelectorAll(
          `[class^="${this.selectors.items}"], [class*=" ${this.selectors.items}"]`,
        );
      ...
      Array.from(items).forEach((item, index) => {
        ...
        try {
          const nameElement = this.getElementByClassPrefix(
            item,
            this.selectors.name,
          );
          const ageElement = this.getElementByClassPrefix(
            item,
            this.selectors.age,
          );
          const educationElement = this.getElementByClassPrefix(
            item,
            this.selectors.education,
          );
          const universityElement = this.getElementByClassPrefix(
            item,
            this.selectors.university,
          );
          const descriptionElement = this.getElementByClassPrefix(
            item,
            this.selectors.description,
          );
          const extraInfo = this.getElementByClassPrefix(
            item,
            this.selectors.extraSelectors,
          );
          //在线状态
          const onlineStatusElement = this.getElementByClassPrefix(
            item,
            this.selectors.onlineStatus,
          );
          ...
          const candidate = {
            ...
            extraInfo:
              [
                {
                  type: "在线状态",
                  value: onlineStatusElement?.textContent?.trim() || "",
                },
                {
                  type: "求职期望",
                  value: descriptionElement?.textContent?.trim() || "",
                },
              ] || [],
          };
  ...
  async clickMatchedItem(element) {
    try {
      const clickElement = this.getElementByClassPrefix(
        element,
        this.selectors.clickTarget,
      );
      if (clickElement) {
        clickElement.click();
        // return true;
      ...
      // 点击确认按钮
      const confirmClickElement = this.getElementByClassPrefix(
        document,
        this.selectors.confirmClickTarget,
      );
  ...
  async clickCandidateDetail(element) {
    try {
      ...
      localStorage.setItem(
        "goodhr-candidate-click-timestamp",
        clickTimestamp.toString(),
      );
      console.log("已添加候选人点击时间戳:", clickTimestamp);

      // 首先尝试点击姓名链接
      const detailLink = this.getElementByClassPrefix(
        element,
        this.detailSelectors.detailLink,
      );
  ...
  async closeDetail() {
    try {
      ...
      const closeButton = document.querySelector(
        `[class*="${this.detailSelectors.closeButton}"]`,
      );
  ...
  async processCommunication(candidate) {
    try {
      ...
      if (
        ...
        !this.communicationConfig.communicationEnabled
      ) {
