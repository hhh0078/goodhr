...
const defaultVisionDetailAnalysisPrompt = `你是资深招聘顾问。请阅读图片中的候选人详情页，并根据岗位要求直接完成打招呼判断。

重要要求：
1. 必须只输出 JSON，不能输出解释、Markdown、代码块或 <think>。
2. 本轮测速暂时不要返回 resume 简历结构，只返回 analysis。
3. analysis.score 为 0-100 数字，用于判断是否适合打招呼。
4. analysis.should_greet 为布尔值，表示是否建议打招呼，判断标准是 score 是否达到打招呼阈值。
5. analysis.reason 控制在 30 字以内，只写最关键判断原因。
6. 如果图片内容无法识别，analysis.score 为 0，analysis.should_greet 为 false。
