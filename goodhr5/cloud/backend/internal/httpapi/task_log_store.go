...
type TaskLogStore interface {
	...
	ListTaskLogs(userEmail string, taskID string) ([]TaskLog, error)
}
...
func (s *MemoryTaskLogStore) ListTaskLogs(userEmail string, taskID string) ([]TaskLog, error) {
	s.mu.Lock()
	defer s.mu.Unlock()

	items := make([]TaskLog, 0)
	for _, log := range s.logs {
		if log.UserEmail != userEmail || log.TaskID != taskID {
			continue
		}
