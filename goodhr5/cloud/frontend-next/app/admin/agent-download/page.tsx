...
import { buildRuntimeInstallPayload, missingRequiredWinRuntimeURLs } from "@/lib/admin-runtime";
...
export default function AgentDownloadPage() {
  ...
  async function updateRuntime() { if (!agentBase) return notify("本地程序未连接", "error"); setLoading(true); try { let config = onboardingConfig; let missing = missingRequiredWinRuntimeURLs(config); if (missing.length) { const fresh = await cloudRequest("/api/onboarding/status"); config = fresh.config || {}; missing = missingRequiredWinRuntimeURLs(config); } if (missing.length) throw new Error(`运行组件下载地址没拿到：${missing.join("、")}。我重新拉了一次还是空，请检查系统配置。`); await localRequest(agentBase, "/api/v1/runtime/install", { method: "POST", body: buildRuntimeInstallPayload(config) }); notify("组件更新任务已完成", "success"); await load(); } catch (error) { notify(error instanceof Error ? error.message : "组件更新失败", "error"); } finally { setLoading(false); } }
