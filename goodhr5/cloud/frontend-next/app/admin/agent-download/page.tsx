...
export default function AgentDownloadPage() {
  const { agentBase, refreshAgent, notify } = useAdmin();
  const [runtime, setRuntime] = useState<any>({});
  const [config, setConfig] = useState<any>({});
  const [loading, setLoading] = useState(false);

  /** load 读取本地运行状态和云端组件配置。 */
  async function load() { if (!agentBase) return; setLoading(true); try { const [runtimeData, appData] = await Promise.all([localRequest(agentBase, "/api/v1/runtime/status"), cloudRequest("/api/system/app-config")]); setRuntime(runtimeData || {}); setConfig(appData.onboarding_config || appData.config?.onboarding_config || appData.config || {}); } catch (error) { notify(error instanceof Error ? error.message : "组件信息读取失败", "error"); } finally { setLoading(false); } }
  useEffect(() => { void load(); }, [agentBase]);

  /** updateRuntime 下载并安装缺失或版本不符的运行组件。 */
  async function updateRuntime() { if (!agentBase) return notify("本地程序未连接", "error"); setLoading(true); try { await localRequest(agentBase, "/api/v1/runtime/install", { method: "POST", body: config }); notify("组件更新任务已完成", "success"); await load(); } catch (error) { notify(error instanceof Error ? error.message : "组件更新失败", "error"); } finally { setLoading(false); } }

  const components = buildComponents(runtime, config);
