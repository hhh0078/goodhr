...
import { FormActionRow, PageHeader, RefreshButton, SectionPanel } from "@/components/admin/AdminUI";
...
export default function HelpPage() {
  ...
  return <><PageHeader title="常见问题" description="查看系统使用说明，或直接向帮助助手描述遇到的问题。" actions={<RefreshButton loading={guideLoading} onClick={() => void loadGuide()} />} />
    <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", xl: "minmax(0, .9fr) minmax(420px, 1.1fr)" }, gap: 2 }}>
      ...
      <SectionPanel><Stack direction="row" sx={{ justifyContent: "space-between", alignItems: "flex-start", gap: 2 }}><Box><Typography component="h2" sx={{ fontSize: 19, fontWeight: 780 }}>AI 帮助助手</Typography><Typography sx={{ mt: 0.5, color: "text.secondary", fontSize: 13 }}>对话只缓存在当前浏览器，最多保留最近 20 轮。</Typography></Box><Button size="small" color="secondary" startIcon={<DeleteSweepRoundedIcon />} disabled={!messages.length || loading} onClick={clearChat}>清空</Button></Stack><Stack ref={chatBody} spacing={1.5} sx={{ mt: 2, minHeight: 360, maxHeight: 560, overflowY: "auto", pr: 0.5 }}>{messages.length ? messages.map((item, index) => <Box key={`${item.role}-${index}`} sx={{ alignSelf: item.role === "user" ? "flex-end" : "flex-start", maxWidth: "88%", p: 1.5, bgcolor: item.role === "user" ? "action.selected" : "action.hover", border: "1px solid", borderColor: "divider", borderRadius: "8px" }}><Typography sx={{ mb: 0.5, color: "text.secondary", fontSize: 11, fontWeight: 800 }}>{item.role === "user" ? "我" : "助手"}</Typography><Typography sx={{ whiteSpace: "pre-wrap", lineHeight: 1.7 }}>{item.content || "正在思考..."}</Typography></Box>) : <Typography color="text.secondary">例如：本地程序连不上怎么办？岗位运行参数分别是什么意思？</Typography>}</Stack><Box sx={{ mt: 2 }}><FormActionRow maxWidth="100%" field={<TextField value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void send(); } }} placeholder="描述你遇到的问题" fullWidth multiline minRows={2} maxRows={5} />} action={<Button variant="contained" aria-label="发送问题" disabled={loading || !input.trim()} onClick={() => void send()} sx={{ minWidth: 48 }}><SendRoundedIcon /></Button>} /></Box></SectionPanel>
    </Box></>;
}
