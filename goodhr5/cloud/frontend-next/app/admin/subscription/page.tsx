...
import { Box, Button, Chip, Dialog, DialogActions, DialogContent, DialogTitle, MenuItem, Pagination, Stack, Tab, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Tabs, TextField, Typography } from "@mui/material";
...
export default function SubscriptionPage() {
  ...
  async function loadSummary() {
    ...
    try {
      ...
      notify(error instanceof Error ? error.message : "订阅信息读取失败，我再试也得先缓缓。", "error");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function loadAIRecords() {
    setAILoading(true);
    try {
      const params = new URLSearchParams({ page: String(aiPage), page_size: String(aiRecordPageSize) });
      const data = await cloudRequest(`/api/ai-wallet/records?${params.toString()}`);
      setAIRecords(Array.isArray(data.records) ? data.records : []);
      setAITotal(Number(data.total || 0));
    } catch (error) {
      notify(error instanceof Error ? error.message : "AI 使用记录读取失败，我小声记下了。", "error");
    } finally {
      setAILoading(false);
    }
  }
  ...
  async function redeem() {
    ...
    if (!value) return;
    try {
      await cloudRequest("/api/subscription/redeem", { method: "POST", body: { code: value } });
      setCode("");
      notify("激活成功，会员时间已到账。", "success");
      await refreshAll();
      await markOnboardingStep(String(user?.email || ""), "subscription_viewed");
    } catch (error) {
      notify(error instanceof Error ? error.message : "激活码没通过，我也有点尴尬。", "error");
    }
  }
  ...
  async function pay(planID: string) {
    ...
    try {
      ...
      notify(error instanceof Error ? error.message : "订单没创建成功，我们再来一次。", "error");
    } finally {
      setPayingPlanID("");
    }
  }
  ...
  async function rechargeAI() {
    ...
    setRecharging(true);
    try {
      const data = await cloudRequest("/api/payment/ai-balance", { method: "POST", body: { amount_yuan: rechargeAmount || "5" } });
      submitPayment(data.payment);
  ...
  return (
    <>
      <PageHeader title="订阅会员" description="会员、AI 余额和账务记录都放这儿，少跑几个页面。" actions={<RefreshButton loading={loading || aiLoading} onClick={() => void refreshAll()} />} />

      <SectionPanel sx={{ mb: 2, bgcolor: "#f7faf8" }}>
        <Typography component="h2" sx={{ fontSize: 20, fontWeight: 820 }}>会员和 AI 余额不是一回事</Typography>
        <Typography sx={{ mt: 1, color: "text.secondary", lineHeight: 1.75 }}>
          会员管“能不能用高级功能”，比如 AI 筛选、详情识别和智能沟通；AI 余额管“内置 AI 实际消耗”，用一次扣一次。简单说：会员是门票，AI 余额是油费，我尽量说人话了。
        </Typography>
      </SectionPanel>

      <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", lg: "repeat(3, minmax(0, 1fr))" }, gap: 2, mb: 3 }}>
        <InfoCard icon={<WorkspacePremiumRoundedIcon />} title="当前会员" value={subscription.member_type || "免费版"} tone={subscription.active ? "dark" : "plain"}>
          <Stack direction="row" spacing={1} sx={{ mt: 1.5, alignItems: "center", flexWrap: "wrap", rowGap: 1 }}>
            <Chip size="small" label={subscription.active ? "会员有效" : "未开通或已到期"} color={subscription.active ? "success" : "default"} />
            <Typography sx={{ color: "text.secondary", fontSize: 13 }}>到期：{formatDate(subscription.expires_at) || "--"}</Typography>
          </Stack>
        </InfoCard>

        <InfoCard icon={<PaidRoundedIcon />} title="AI 余额" value={`￥${wallet.balance || "0.0000"}`}>
          <Stack spacing={1.25} sx={{ mt: 1.5 }}>
            <Chip size="small" label={`当前模型：${modelLabel}`} onClick={openModelDialog} sx={{ width: "fit-content", bgcolor: "#eef6f0", color: "#2f6f4f", fontWeight: 700 }} />
            <Stack direction="row" spacing={1} sx={{ flexWrap: "wrap", rowGap: 1 }}>
              <Button size="small" variant="contained" disabled={recharging} onClick={() => setRechargeDialogOpen(true)}>{recharging ? "下单中" : "充值"}</Button>
              <Button size="small" variant="outlined" onClick={() => setRecordTab("ai")}>看记录</Button>
            </Stack>
          </Stack>
        </InfoCard>

        <InfoCard icon={<CreditCardRoundedIcon />} title="会员激活码" value="兑换会员时间">
          <Box sx={{ mt: 1.5 }}>
            <FormActionRow field={<TextField size="small" label="会员激活码" value={code} onChange={(event) => setCode(event.target.value)} fullWidth />} action={<Button variant="contained" disabled={!code.trim()} onClick={() => void redeem()}>激活</Button>} maxWidth={520} />
          </Box>
        </InfoCard>
      </Box>

      <Box sx={{ mb: 2.25 }}>
        <Typography component="h2" sx={{ fontSize: 22, fontWeight: 780 }}>选择会员套餐</Typography>
        <Typography sx={{ mt: 0.5, color: "text.secondary" }}>未到期时购买会从当前到期时间继续增加天数。</Typography>
      </Box>
      ...
      <SectionPanel sx={{ mt: 2 }}>
        <Stack direction={{ xs: "column", sm: "row" }} sx={{ justifyContent: "space-between", gap: 1, alignItems: { sm: "center" } }}>
          <Typography component="h2" sx={{ fontSize: 19, fontWeight: 760 }}>账务记录</Typography>
          <Tabs value={recordTab} onChange={(_, value) => setRecordTab(value)}>
            <Tab value="payments" label="支付记录" />
            <Tab value="ai" label="AI 使用记录" />
          </Tabs>
        </Stack>
        {recordTab === "payments" ? <PaymentRecords orders={orders} loading={loading} /> : <AIRecordList records={aiRecords} total={aiTotal} page={aiPage} loading={aiLoading} onPageChange={setAIPage} />}
      </SectionPanel>

      <Dialog open={modelDialogOpen} onClose={() => setModelDialogOpen(false)} fullWidth maxWidth="xs">
        <DialogTitle>选择当前模型</DialogTitle>
        <DialogContent>
          <TextField select fullWidth label="当前模型" value={selectedModel} onChange={(event) => setSelectedModel(event.target.value)} sx={{ mt: 1 }}>
        ...
        <DialogActions>
          ...
          <Button variant="contained" disabled={savingModel} onClick={() => void saveAIModel()}>{savingModel ? "保存中" : "保存模型"}</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={rechargeDialogOpen} onClose={() => setRechargeDialogOpen(false)} fullWidth maxWidth="xs">
        ...
        <DialogContent>
          <TextField fullWidth label="充值金额（元）" value={rechargeAmount} onChange={(event) => setRechargeAmount(event.target.value)} slotProps={{ htmlInput: { inputMode: "decimal" } }} sx={{ mt: 1 }} />
          <Typography sx={{ mt: 1.25, color: "text.secondary", fontSize: 13 }}>默认 5 元，够先跑一阵。填完我就去创建支付订单。</Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setRechargeDialogOpen(false)}>先不充</Button>
          <Button variant="contained" disabled={recharging} onClick={() => void rechargeAI()}>{recharging ? "下单中" : "去支付"}</Button>
        </DialogActions>
      </Dialog>
    </>
  );
...
function InfoCard({ icon, title, value, tone = "plain", children }: { icon: React.ReactNode; title: string; value: string; tone?: "plain" | "dark"; children?: React.ReactNode }) {
  const dark = tone === "dark";
  return <SectionPanel sx={{ minHeight: 190, bgcolor: dark ? "#15271e" : "#fbfdfc", color: dark ? "#f8f1da" : "text.primary", borderColor: dark ? "#4d5a48" : "divider" }}>
    <Stack direction="row" spacing={1.5} sx={{ alignItems: "center" }}>
      <Box sx={{ width: 44, height: 44, borderRadius: "8px", display: "grid", placeItems: "center", bgcolor: dark ? "#c9a55d" : "#e9f2ec", color: dark ? "#1b241e" : "#1e6545" }}>{icon}</Box>
      <Box sx={{ minWidth: 0 }}>
        <Typography sx={{ fontSize: 13, opacity: dark ? 0.72 : 0.68 }}>{title}</Typography>
        <Typography sx={{ mt: 0.5, fontSize: 24, fontWeight: 820, wordBreak: "break-word" }}>{value}</Typography>
      </Box>
    </Stack>
    {children}
  </SectionPanel>;
...
function PaymentRecords({ orders, loading }: { orders: any[]; loading: boolean }) {
  return orders.length ? <Stack sx={{ mt: 1.5 }}>{orders.map((order) => <Stack key={order.order_no} direction={{ xs: "column", md: "row" }} spacing={1.5} sx={{ py: 1.5, borderBottom: "1px solid", borderColor: "divider", alignItems: { md: "center" } }}><Box sx={{ flex: 1, minWidth: 0 }}><Typography sx={{ fontWeight: 700 }}>{order.plan_name || order.order_type || "支付订单"}</Typography><Typography noWrap sx={{ color: "text.secondary", fontSize: 12 }}>{order.order_no}</Typography></Box><Typography sx={{ minWidth: 80, fontWeight: 700 }}>￥{(Number(order.amount_cents || 0) / 100).toFixed(2)}</Typography><Chip size="small" color={order.status === "paid" ? "success" : "default"} label={statusText(order.status)} /><Typography sx={{ color: "text.secondary", fontSize: 13, minWidth: 168 }}>{formatDate(order.created_at)}</Typography></Stack>)}</Stack> : <EmptyState text={loading ? "正在读取支付记录" : "暂无支付记录"} />;
}

/** AIRecordList 展示 AI 余额流水并分页。 */
function AIRecordList({ records, total, page, loading, onPageChange }: { records: any[]; total: number; page: number; loading: boolean; onPageChange: (value: number) => void }) {
  const pageCount = Math.max(1, Math.ceil(total / aiRecordPageSize));
  return records.length ? <>
    <TableContainer sx={{ mt: 1.5, display: { xs: "none", md: "block" } }}>
      <Table size="small">
        <TableHead><TableRow sx={{ bgcolor: "#f6faf7" }}><TableCell>类型</TableCell><TableCell>金额</TableCell><TableCell>余额</TableCell><TableCell>模型 / Token</TableCell><TableCell>说明</TableCell><TableCell>时间</TableCell></TableRow></TableHead>
        <TableBody>{records.map((record) => <AIRecordRow key={record.id || `${record.created_at}-${record.reason}`} record={record} />)}</TableBody>
      </Table>
    </TableContainer>
    <Stack spacing={1.25} sx={{ mt: 1.5, display: { xs: "flex", md: "none" } }}>{records.map((record) => <AIRecordCard key={record.id || `${record.created_at}-${record.reason}`} record={record} />)}</Stack>
    <Pagination page={page} count={pageCount} onChange={(_, value) => onPageChange(value)} sx={{ mt: 2, display: "flex", justifyContent: "center" }} />
  </> : <EmptyState text={loading ? "正在读取 AI 使用记录" : "这里暂时空空的，等你开始使用内置 AI 后我再认真记账。"} />;
}

/** AIRecordRow 展示桌面端单条 AI 记录。 */
function AIRecordRow({ record }: { record: any }) {
  return <TableRow hover><TableCell><CategoryChip category={record.category} /></TableCell><TableCell><AmountText units={Number(record.change_units || 0)} value={record.change} /></TableCell><TableCell>￥{record.balance_after || "0.00"}</TableCell><TableCell><TokenInfo record={record} /></TableCell><TableCell><ReasonText record={record} /></TableCell><TableCell>{formatDate(record.created_at)}</TableCell></TableRow>;
}

/** AIRecordCard 展示移动端单条 AI 记录。 */
function AIRecordCard({ record }: { record: any }) {
  return <Box sx={{ p: 1.5, border: "1px solid", borderColor: "divider", borderRadius: "8px", bgcolor: "#fbfdfc" }}><Stack direction="row" sx={{ justifyContent: "space-between", gap: 1 }}><CategoryChip category={record.category} /><AmountText units={Number(record.change_units || 0)} value={record.change} /></Stack><Typography sx={{ mt: 1, color: "text.secondary", fontSize: 13 }}><ReasonText record={record} /></Typography><Typography sx={{ mt: 0.75, color: "text.secondary", fontSize: 12 }}><TokenInfo record={record} /> · 余额 ￥{record.balance_after || "0.00"} · {formatDate(record.created_at)}</Typography></Box>;
}

/** CategoryChip 展示流水类别。 */
function CategoryChip({ category }: { category: string }) {
  const labelMap: Record<string, string> = { ai_usage: "AI调用", recharge: "充值", signup_bonus: "注册赠送", admin_adjust: "后台调整" };
  const color = category === "ai_usage" ? "warning" : category === "admin_adjust" ? "info" : "success";
  return <Chip size="small" color={color as any} variant="outlined" label={labelMap[category] || category || "记录"} />;
}
...
function AmountText({ units, value }: { units: number; value: string }) {
  ...
  return <Stack direction="row" spacing={0.4} sx={{ alignItems: "center", color: positive ? "#15804f" : "#b45309", fontWeight: 800 }}><Icon sx={{ fontSize: 16 }} /><Typography sx={{ fontWeight: 800 }}>{positive ? "+" : ""}￥{value || "0.00"}</Typography></Stack>;
}

/** TokenInfo 展示模型和 token 用量。 */
function TokenInfo({ record }: { record: any }) {
  const tokens = Number(record.total_tokens || 0);
  return <>{record.model_id || "无模型"}{tokens ? ` / ${tokens} tokens` : ""}</>;
}

/** ReasonText 展示记录说明和关联订单号。 */
function ReasonText({ record }: { record: any }) {
  return <>{record.reason || "暂无说明"}{record.related_order_no ? `（订单 ${record.related_order_no}）` : ""}</>;
}
