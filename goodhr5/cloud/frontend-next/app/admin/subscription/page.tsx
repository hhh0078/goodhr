...
export default function SubscriptionPage() {
  ...
  /** load 读取订阅、套餐和支付记录。 */
  async function load() { setLoading(true); try { const [planData, orderData] = await Promise.all([cloudRequest("/api/subscription/plans", { auth: false }), cloudRequest("/api/payment/orders")]); setPlans(planData.plans || []); setOrders(orderData.orders || []); } catch (error) { notify(error instanceof Error ? error.message : "订阅信息读取失败", "error"); } finally { setLoading(false); } }
  useEffect(() => { void load(); }, []);

  /** redeem 兑换会员激活码。 */
  async function redeem() { if (!code.trim()) return; try { await cloudRequest("/api/activation-codes/redeem", { method: "POST", body: { code: code.trim() } }); await refreshSession(); setCode(""); notify("激活成功，会员时间已增加", "success"); } catch (error) { notify(error instanceof Error ? error.message : "激活失败", "error"); } }
