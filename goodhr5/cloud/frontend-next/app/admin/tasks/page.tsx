...
const emptyForm = { id: "", name: "", platform_account_id: "", position_id: "", match_limit: 50, enable_sound: false, enable_thinking: false };
...
export default function TasksPage() {
  ...
  useEffect(() => {
    const ids = Object.keys(runningTaskIDs).filter((taskID) => runningTaskIDs[taskID]);
  ...
  const taskList = useMemo(() => tasks.map((task) => ({ ...task, status: runningTaskIDs[task.id] ? "running" : task.status })), [runningTaskIDs, tasks]);
  ...
  async function save() {
    ...
    setLoading(true);
    try {
      const payload = { name: form.name.trim() || `${position.name}招聘任务`, platform_id: account.platform_id, platform_account_id: account.id, position_id: position.id, mode: position.common_config?.mode_default || "keyword", match_limit: Math.max(1, Number(form.match_limit || 50)), enable_sound: form.enable_sound, enable_thinking: form.enable_thinking, position_snapshot: position };
      await cloudRequest(form.id ? `/api/tasks/${encodeURIComponent(form.id)}` : "/api/tasks", { method: form.id ? "PUT" : "POST", body: payload });
      notify(form.id ? "任务已更新" : "任务已创建", "success"); setForm({ ...emptyForm }); setShowForm(false); await load();
    } catch (error) { notify(error instanceof Error ? error.message : "保存任务失败", "error"); }
    finally { setLoading(false); }
  }
  ...
  function openEdit(task: any) {
    setForm({ id: task.id || "", name: task.name || "", platform_account_id: task.platform_account_id || task.platform_account?.id || "", position_id: task.position_id || task.position?.id || "", match_limit: Number(task.match_limit || 50), enable_sound: Boolean(task.enable_sound), enable_thinking: Boolean(task.enable_thinking) });
    setShowForm(true);
  }
  ...
  async function stop(task: any) {
    ...
    try { await localRequest(agentBase, `/api/v1/local/tasks/${encodeURIComponent(task.id)}/stop`, { method: "POST", body: { cloud_api_base: CLOUD_API_BASE, token: getToken() } }); setRunningTaskIDs((current) => ({ ...current, [task.id]: false })); notify("任务已停止，浏览器保持打开", "success"); await load(); } catch (error) { notify(error instanceof Error ? error.message : "停止任务失败", "error"); }
  }
  ...
  async function refreshLocalTaskStatus(taskID: string) {
    if (!agentBase) return;
    try {
      const data = await localRequest(agentBase, `/api/v1/local/tasks/${encodeURIComponent(taskID)}/status`);
  ...
  async function clearLogs(taskID: string) {
    if (!agentBase || !(await confirm("清空任务日志", "确认清空该任务的本地日志吗？"))) return;
    try { await localRequest(agentBase, `/api/v1/local/tasks/${encodeURIComponent(taskID)}/logs`, { method: "DELETE" }); setLogs((current) => ({ ...current, [taskID]: [] })); notify("任务日志已清空", "success"); } catch (error) { notify(error instanceof Error ? error.message : "清空日志失败", "error"); }
  }

  return <><PageHeader title="任务列表" description="创建任务后由本地程序运行，云端保存任务和岗位配置。" actions={<><Button variant="contained" startIcon={<AddRoundedIcon />} onClick={openCreate}>创建任务</Button><RefreshButton loading={loading} onClick={() => void load()} /></>} /><SectionPanel>{taskList.length ? <Stack spacing={1.5}>{taskList.map((task) => <Box key={task.id} sx={{ borderBottom: "1px solid", borderColor: "divider", pb: 1.5 }}><Stack direction={{ xs: "column", md: "row" }} spacing={2} sx={{ alignItems: { md: "center" } }}><Box sx={{ flex: 1 }}><Stack direction="row" spacing={1} sx={{ alignItems: "center" }}><Typography sx={{ fontWeight: 760 }}>{task.name || "未命名任务"}</Typography><Chip size="small" color={task.status === "running" ? "success" : "default"} label={statusLabel(task.status)} /></Stack><Typography sx={{ mt: 0.5, color: "text.secondary", fontSize: 13 }}>{task.platform_account?.display_name || "未选择账号"} · 本次上限 {Number(task.match_limit || 50)} · 总计 {Number(task.greeted_count || 0)} · 今日 {Number(task.today_greeted_count || 0)} · 本次 {Number(task.current_run_greeted_count || 0)} · 提示音 {task.enable_sound ? "开" : "关"}</Typography><Typography sx={{ mt: 0.5, color: "text.secondary", fontSize: 12 }}>更新时间：{formatDate(task.updated_at)}</Typography></Box><Stack direction="row" spacing={0.5} sx={{ flexWrap: "wrap" }}>{task.status === "running" ? <Button color="warning" startIcon={<StopRoundedIcon />} onClick={() => void stop(task)}>停止</Button> : <Button variant="contained" startIcon={<PlayArrowRoundedIcon />} onClick={() => void run(task)}>开始</Button>}<Button startIcon={<EditRoundedIcon />} disabled={task.status === "running"} onClick={() => openEdit(task)}>编辑</Button><Button component={Link} href={`/admin/resumes?task_id=${encodeURIComponent(task.id)}`}>简历</Button><Button onClick={() => void loadLogs(task.id)}>日志</Button><Button color="error" startIcon={<DeleteOutlineRoundedIcon />} onClick={() => void remove(task)}>删除</Button></Stack></Stack>{logs[task.id] ? <Accordion elevation={0} disableGutters sx={{ mt: 1, bgcolor: "#f7faf8" }}><AccordionSummary expandIcon={<ExpandMoreRoundedIcon />}><Typography>本地任务日志（{logs[task.id].length}）</Typography></AccordionSummary><AccordionDetails><Stack direction="row" sx={{ mb: 1, justifyContent: "flex-end" }}><Button color="error" size="small" onClick={() => void clearLogs(task.id)}>清空日志</Button></Stack><Box component="pre" sx={{ m: 0, maxHeight: 360, overflow: "auto", whiteSpace: "pre-wrap", fontSize: 12, lineHeight: 1.7 }}>{logs[task.id].map((item) => `${formatDate(item.created_at || item.time)}  ${item.message || item.msg || ""}`).join("\n") || "暂无日志"}</Box></AccordionDetails></Accordion> : null}</Box>)}</Stack> : <EmptyState text="暂无招聘任务" />}</SectionPanel><AdminDialog open={showForm} title={form.id ? "编辑招聘任务" : "创建招聘任务"} description="账号决定招聘平台，岗位模板决定筛选和详情识别方式。" confirmText={form.id ? "保存修改" : "创建任务"} loading={loading} maxWidth="md" onClose={() => setShowForm(false)} onConfirm={() => void save()}><Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "repeat(2, 1fr)" }, gap: 2 }}><TextField label="任务名称" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="不填写则按岗位自动生成" /><TextField select label="平台账号" value={form.platform_account_id} onChange={(event) => { const account = accounts.find((item) => item.id === event.target.value); const validPosition = positions.find((item) => item.id === form.position_id && item.platform_id === account?.platform_id); setForm({ ...form, platform_account_id: event.target.value, position_id: validPosition ? form.position_id : "" }); }}>{accounts.map((item) => <MenuItem key={item.id} value={item.id}>{item.display_name} · {item.platform_id}</MenuItem>)}</TextField><TextField select label="岗位模板" value={form.position_id} onChange={(event) => setForm({ ...form, position_id: event.target.value })}>{positions.filter((item) => { const account = accounts.find((current) => current.id === form.platform_account_id); return !account || !item.platform_id || item.platform_id === account.platform_id; }).map((item) => <MenuItem key={item.id} value={item.id}>{item.name}</MenuItem>)}</TextField><TextField label="本次打招呼上限" type="number" value={form.match_limit} onChange={(event) => setForm({ ...form, match_limit: Number(event.target.value) })} helperText="本次运行达到该数量后自动停止，默认 50。" /><FormControlLabel control={<Switch checked={form.enable_sound} onChange={(event) => setForm({ ...form, enable_sound: event.target.checked })} />} label="打招呼成功后播放提示音" /><FormControlLabel control={<Switch checked={form.enable_thinking} onChange={(event) => setForm({ ...form, enable_thinking: event.target.checked })} />} label="在浏览器显示 AI 思考过程" /></Box></AdminDialog></>;
}
