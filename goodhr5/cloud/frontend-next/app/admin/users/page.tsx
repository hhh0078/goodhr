...
export default function UsersPage() {
  ...
  async function adjust() {
    if (!Number.isFinite(form.days) || form.days === 0) return notify("天数不能为 0，正数增加，负数减少", "warning");
    try {
  ...
  return <>
    ...
    <AdminDialog open={dialogOpen} title="调整会员时间" description={`当前用户：${form.email || "--"}`} confirmText="确认调整" loading={loading} onClose={() => setDialogOpen(false)} onConfirm={() => void adjust()}>
      <Stack spacing={2}>
        ...
        <TextField label="调整天数" type="number" value={form.days} onChange={(event) => setForm({ ...form, days: Number(event.target.value) })} fullWidth slotProps={{ htmlInput: { step: 1 } }} helperText="正数增加，负数减少，不能为 0。" />
