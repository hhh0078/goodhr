  ...
  return <MarketingShell eyebrow="永久免费 + AI 订阅" title="关键词免费用，AI 能力按需升级" description="免费版可以跑完整的基础招聘流程。需要 AI 判断、继续沟通和邀约面试时，再升级 Plus。">
    <Box component="section" sx={{ pb: { xs: 8, md: 12 } }}><Container maxWidth="lg">
      <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "repeat(2, 1fr)", lg: "repeat(4, 1fr)" }, gap: 2 }}>
        {plans.map((plan, index) => <Paper key={plan.id} variant="outlined" sx={{ p: 3, borderRadius: "8px", borderColor: index === 1 ? "primary.main" : "divider", boxShadow: index === 1 ? "0 18px 48px rgba(21,154,98,.12)" : "none" }}>
          <Typography component="h2" sx={{ fontSize: 20, fontWeight: 760 }}>{plan.name}</Typography><Stack direction="row" spacing={0.75} sx={{ mt: 2, alignItems: "baseline" }}><Typography sx={{ fontSize: 42, fontWeight: 800 }}>￥{finalPrice(plan)}</Typography><Typography color="text.secondary">{planUnit(plan)}</Typography></Stack>{plan.discountAmount > 0 ? <Typography sx={{ mt: 0.5, color: "text.secondary", textDecoration: "line-through" }}>原价 ￥{plan.originalPrice}</Typography> : null}<Typography sx={{ mt: 1.5, minHeight: 52, color: "text.secondary", lineHeight: 1.65 }}>{plan.description}</Typography><Stack spacing={1.25} sx={{ mt: 3 }}>{plan.features.map((feature) => <Stack key={feature} direction="row" spacing={1} sx={{ alignItems: "center" }}><CheckRoundedIcon color="primary" fontSize="small" /><Typography>{feature}</Typography></Stack>)}</Stack><Button component="a" href="/login" variant={index === 1 ? "contained" : "outlined"} fullWidth sx={{ mt: 3 }}>立即使用</Button>
        </Paper>)}
      </Box>
...
function freePlan(): PublicPlanData {
  return { id: "free", name: "永久免费版", memberType: "free", durationDays: 0, originalPrice: 0, discountAmount: 0, description: "适合先用关键词规则跑通招聘流程。", features: ["关键词筛选", "基础任务", "平台账号管理", "自动打招呼"] };
}
