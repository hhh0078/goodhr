...
export default function RequiredRuntimeInstaller({ agentBase, onboardingConfig, notify }: RequiredRuntimeInstallerProps) {
	...
	return <Dialog open={visible} fullWidth maxWidth="sm" onClose={() => undefined}><DialogContent sx={{ p: { xs: 2.5, sm: 3 } }}><Stack direction="row" spacing={1.5} sx={{ alignItems: "center", justifyContent: "space-between" }}><Box><Typography component="h2" sx={{ fontSize: 22, fontWeight: 800 }}>安装必要组件</Typography><Typography sx={{ mt: 0.75, color: "text.secondary", lineHeight: 1.7 }}>本地程序需要这些组件才能控制浏览器。没装好前，我先拦一下，免得岗位运行跑到半路掉链子。</Typography></Box><Chip color={running ? "warning" : "error"} label={running ? "安装中" : "必须完成"} /></Stack><Stack spacing={1.25} sx={{ mt: 2.5 }}>{components.map((item) => <Stack key={item.key} direction="row" sx={{ alignItems: "center", justifyContent: "space-between", p: 1.25, border: "1px solid", borderColor: "divider", borderRadius: "8px", bgcolor: item.installed ? "success.light" : "#fff8ed" }}><Typography sx={{ fontWeight: 700 }}>{item.name}</Typography><Chip size="small" color={item.installed ? "success" : "warning"} label={item.installed ? "已可用" : "未安装"} /></Stack>)}</Stack>{running || progress?.message ? <Box sx={{ mt: 2.5, p: 1.5, border: "1px solid", borderColor: "divider", borderRadius: "8px", bgcolor: "action.hover" }}><Stack direction="row" sx={{ alignItems: "center", justifyContent: "space-between", mb: 1 }}><Typography sx={{ fontWeight: 760 }}>{runtimeProgressTitle(progress?.stage)}</Typography><Typography sx={{ color: "text.secondary", fontSize: 13 }}>{percent}%</Typography></Stack><LinearProgress variant="determinate" value={percent} sx={{ height: 8, borderRadius: 999 }} /><Typography sx={{ mt: 1, color: "text.secondary", fontSize: 13 }}>{progress?.message || "正在准备安装"}</Typography>{progressBytes ? <Typography sx={{ mt: 0.5, color: "text.secondary", fontSize: 12 }}>{progressBytes}</Typography> : null}</Box> : null}{error ? <Typography sx={{ mt: 2, color: "error.main", fontSize: 13 }}>{error}</Typography> : null}<Button fullWidth variant="contained" size="large" startIcon={<DownloadRoundedIcon />} disabled={running} onClick={() => void installRuntime()} sx={{ mt: 2.5 }}>{running ? "正在安装..." : "安装必要组件"}</Button></DialogContent></Dialog>;
}
