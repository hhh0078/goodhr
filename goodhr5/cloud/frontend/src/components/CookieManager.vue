      ...
      <div class="form-grid">
        ...
        <label
          >名称<input
            ...
            placeholder="我的Boss账号cookie"
        /></label>
      </div>
      <p v-if="msg" :class="msgType">{{ msg }}</p>
      <div class="actions">
        <button :disabled="loading || !form.displayName" @click="createCookie">
          新增 Cookie
        </button>
      </div>
      <p v-if="cookies.length === 0" class="hint" style="margin-top: 12px">
        暂无 cookie，新增后将自动导航到登录页获取。
      </p>
      <div v-else class="card-list" style="margin-top: 12px">
        <article v-for="c in cookies" :key="c.id" class="card">
          <div>
            <strong>{{ c.display_name || c.id }}</strong>
            <p class="card-meta">
              {{ c.platform_id }} | {{ c.cookie_type }} | {{ c.status }}
            </p>
          </div>
          <div class="card-actions">
            <button class="ghost danger" @click="deleteCookie(c.id)">
              删除
            </button>
          </div>
        </article>
...
const api = (path: string, opts?: RequestInit) =>
  fetch(
    `${(window as any).GOODHR_CLOUD_API || "http://127.0.0.1:8084"}${path}`,
    {
      headers: {
...
async function createCookie() {
  ...
  msg.value = "";
  try {
    const d = await api("/api/cookies/create", {
      method: "POST",
      body: JSON.stringify({
