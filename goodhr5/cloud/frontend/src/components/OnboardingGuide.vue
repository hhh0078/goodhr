...
<template>
  <section class="panel onboarding-panel">
    <div class="panel-header">
      <div>
        <h2>新手教学</h2>
        <p class="hint">按顺序完成这些步骤，全部完成后这里会自动消失。</p>
      </div>
    ...
    <div class="onboarding-grid">
      <article
        v-for="card in cards"
        :key="card.key"
        :class="['guide-card', { done: isDone(card.key), active: card.key === activeKey }]"
      >
        ...
        <button v-if="card.key === 'local_agent' && !isDone(card.key)" class="ghost primary" @click="openDownload">
          下载本地程序
        </button>
        <button v-else-if="card.menu" class="ghost" @click="$emit('go', card.menu)">
          去完成
        </button>
      </article>
    </div>
...
const cards = [
  {
    ...
    title: "确认本地程序已启动",
    menu: "agent",
    description: "本地 GoodHR 程序是核心组件，浏览器控制、截图和 OCR 都依赖它。",
    tips: ["先启动本地 GoodHR 程序", "前端检测 health 成功后会自动完成", "检测失败时请下载并启动本地程序"],
  },
  {
    ...
    tips: ["左侧进入平台账号", "点击新增，再点登录并获取Cookie", "扫码登录完成后填写名称并保存账号"],
  },
  {
    ...
    tips: ["左侧进入岗位模板", "点击新建模板", "填写岗位名称、岗位要求或关键词后保存"],
  },
  {
    key: "personal_config",
    index: "04",
    ...
    tips: ["API地址可填 https://api.deepseek.com/chat/completions", "模型可填 deepseek-v4-flash 或 deepseek-v4-pro", "保存后 API Key 会隐藏"],
  },
  {
    key: "task_started",
    index: "05",
    ...
    tips: ["左侧进入任务列表", "点击创建任务并选择账号", "任务开始成功后自动完成这一步"],
  },
  {
...
const doneCount = computed(() => ONBOARDING_STEPS.filter((step) => isDone(step)).length);
const activeKey = computed(() => cards.find((card) => !isDone(card.key))?.key || "");
