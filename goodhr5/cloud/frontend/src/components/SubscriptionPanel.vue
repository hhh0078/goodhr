...
<template>
  <section class="panel">
    ...
    <div style="margin-bottom: 20px">
      <p>
        1
        充值后，您在任何时间都可以退费。我们将按原价/套餐天数作为单价*剩余天数=退费金额。还会收取5%的退费手续费，因为我们使用的充值渠道是有百分之5%的手续费。
      </p>
      <p>2 如果您还未到期、充值时我们会从到期时间开始增加天数</p>
      <p>
        3
        购买过永久会员的用户、以及充值过未使用完的可以联系作者直接抵扣升级，继续充值视为您同意充值协议。
      </p>
    </div>
    ...
    <div class="plan-grid">
      <article v-for="plan in plans" :key="plan.id" class="plan-card">
        ...
        <button
          ...
          @click="pay(plan)"
          v-if="Number(plan.original_price || 0) > 0"
        >
