  ...
  <section class="panel">
    ...
    <div class="form-grid">
      <label
        >平台
        <select
          v-model="tasks.form.value.platformId"
    ...
    <div v-if="selectedPosition" class="snapshot">
      ...
      <p class="snapshot-meta">
        {{ selectedPosition.is_and_mode ? "AND 匹配" : "OR 匹配" }}
      </p>
      <p class="snapshot-meta">
        关键词：{{ (selectedPosition.keywords || []).join(" / ") || "无" }}
      </p>
      <p class="snapshot-meta">
        排除词：{{
          (selectedPosition.exclude_keywords || []).join(" / ") || "无"
        }}
      </p>
    </div>
...
const filteredAccounts = computed(() =>
  accounts.value.filter(
    (a) => a.platform_id === props.tasks.form.value.platformId,
  ),
);
const posList =
  typeof props.positions === "object" && !Array.isArray(props.positions)
    ? (props.positions as any)?.positions || []
    : (props.positions as any[]) || [];
const selectedPosition = computed(
  () =>
    posList.find((p: any) => p.id === props.tasks.form.value.positionId) ||
    null,
);
...
function onPlatformChange() {
  props.tasks.form.value.platformAccountId = "";
}
onMounted(() => {
