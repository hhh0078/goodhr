...
export async function startTaskWS(base: string, taskID: string, payload: any) {
  return req(base, `/api/v1/tasks/${encodeURIComponent(taskID)}/start-ws`, { method: 'POST', body: payload })
}
...
export async function stopTaskWS(base: string, taskID: string, payload: any) {
  return req(base, `/api/v1/tasks/${encodeURIComponent(taskID)}/stop-ws`, { method: 'POST', body: payload })
}
