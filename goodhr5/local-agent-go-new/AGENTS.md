...
- 云端提供登录、岗位、会员、余额、团队公司档案、自动回复配置，以及自动回复需要持久化的候选人、聊天、简历附件元数据、确认项和 AI 审计数据。
- 自动回复的页面流程、AI 工具循环和发送决策只在本地 Go 编排；真实简历附件先上传云端，再由云端 Go 读取文件、调用 AI 结构化并严格校验字段。云端不得操控浏览器或运行自动回复主流程。
- 主动打招呼只有岗位开启 `output_structured_resume` 后，才允许把 AI 完整流式响应中的结构化简历异步同步到云端；自动回复开关开启后，视为明确允许当前岗位同步聊天、简历和附件。
- 主动打招呼的 score、reason 等评分字段完整后主流程必须立即继续，不能等待结构化简历等剩余字段；后台同步失败只记录日志。
- 用户可见的 AI 分析、自动回复 AI 工具调用和结果摘要必须进入本地全局日志；不要再为控制台单独做岗位级 AI 记录入口。
- 自动回复附件文件保存到云端持久化目录，数据库只保存受权限保护的路径、哈希、类型、大小和来源；Cookie、Profile、截图和页面运行数据仍不得上传。
- 自动回复简历结构化必须以云端保存的真实附件正文为主，在线简历只能补充附件缺失字段；严禁只拿页面在线简历冒充附件解析结果。
- 平台 URL、页面行为和选择器只来自 `internal/platform/{platform}/config.json`。
- Go 使用 `go:embed` 把四个平台配置随程序发布，启动任务时不得请求、合并或覆盖云端平台配置。
- 选择器仍统一使用 `SelectorSpec`，不得把 CSS 选择器写进公共流程、Worker 或数据库迁移。

自动回复的长期决定和开发进度分别见：

- `docs/auto-reply-decisions.md`
- `docs/auto-reply-roadmap.md`
...
主动打招呼未明确配置扫描批数时，不得使用固定默认批数自动结束。流程必须持续加载下一批候选人，直到达到岗位本次打招呼上限、平台确认没有更多候选人、用户手动停止、云端状态不允许继续，或统一错误策略触发。
...
AI 请求必须先发送稳定的 `system` 消息，再发送包含本次任务数据的 `user` 消息，避免动态内容破坏固定提示词的缓存前缀。

- `system` 只能放角色、评分规则、输出格式、结构化数据格式和其他固定约束。
- 岗位要求、候选人资料、简历详情、图片、聊天记录等动态内容只能放在 `user`。
- 严禁通过字符串拼接或模板替换，把岗位要求、候选人信息等动态变量塞入固定提示词。
- 固定消息的文字、字段顺序和消息顺序必须保持稳定；没有规则变化时，不得为了文案调整随意改动。
- 岗位自定义提示词可以作为独立的 `system` 消息，但其中不得替换岗位要求、候选人信息等动态变量。
- 图片必须与本次动态文字一起放在 `user` 消息中。
- 本规则适用于基础预分析、最终评分、图片分析和自动回复。
...
- 选择使用哪些 Worker 封装能力。
- 从当前平台目录的 `config.json` 读取 URL、选择器和页面规则。
- 把通用候选人模型转换为平台页面动作。

平台层禁止：

- 在 Go 源码、Worker 或云端数据库中重复硬编码平台选择器。
...
主动打招呼流程需要索要资料或发送首次招呼语时，禁止跳转独立“沟通/消息”页面。统一留在推荐页，按以下顺序复用公共能力：

```text
当前聊天框属于候选人 -> 直接复用
否则候选人卡片有“继续沟通” -> 直接点击打开聊天框
否则首次消息 -> 推荐页右侧联系人入口
  -> 联系人侧边栏
  -> 查找并点击当前候选人打开聊天框
  -> 索要资料或发送消息
...
入口、侧边栏、联系人项和关闭按钮只在各平台 `config.json` 中维护；公共流程只引用 `candidate.contact_*` 逻辑键。不是首次消息时必须优先点击当前推荐卡片的“继续沟通”，禁止多余打开联系人侧边栏；只有卡片还没有“继续沟通”时才打开右侧联系人列表，整个过程不得离开推荐页。

公共平台接口定义能力，平台目录实现动作顺序，同目录 `config.json` 提供 URL、页面行为和选择器。禁止为了省事把平台差异塞回主流程或 Worker。
