...
- 云端只提供登录、岗位、会员、余额和用户运行设置等业务数据。
- 岗位开启 `output_structured_resume` 后，AI 完整流式响应中的结构化简历允许由 Go 在后台异步同步到云端简历库；未开启时禁止上传。
- score、reason 等评分字段完整后主流程必须立即继续，不能等待结构化简历等剩余字段；后台同步失败只记录日志。
