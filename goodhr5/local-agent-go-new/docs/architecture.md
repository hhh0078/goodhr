...
评分字段和结构化简历采用两段式处理：score、reason 完整后主流程立即继续；剩余结构化字段由后台接收并同步云端。未开启结构化简历时不得上传候选人详情。
...
同一个持久化 Profile 使用稳定的 Camoufox 指纹文件。语言与时区固定为 zh-CN 和 Asia/Shanghai，不启用按代理 IP 匹配地理位置的 geoip（用户与平台均在中国大陆，无动态匹配需求）。
