...
- 从当前平台目录的 `config.json` 读取 URL、页面行为和选择器。
...
- `config.json`：带中文属性说明的本地运行配置和迁移核对表，是该平台 URL、页面行为和选择器的唯一来源。
...
公共接口只描述“平台能做什么”。每个平台文件决定用哪些逻辑键和动作顺序，同目录 `config.json` 提供选择器值。任何新增平台必须先实现完整接口，不能在主流程里用平台名加例外分支。
...
- 同账号隔离。
- 登录状态和占用锁。
- 初始化并幂等维护 GoodHR、BOSS、猎聘和智联等手动导航书签。
- 保留用户已有书签，并配置 Chromium 在所有页面显示书签栏。

默认书签只用于方便用户手动导航，不参与平台自动化流程。这里不直接控制浏览器页面，也不修改默认搜索引擎和 Chromium 受保护偏好。
