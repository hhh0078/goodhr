...
- AI 岗位要求云端会员状态同时满足 `active=true` 和 `allow_ai=true`；自动回复还必须满足 `allow_auto_reply=true`，当前只有 Max 全能版开放。
- 本地检查是云端启动检查后的第二道保护，不能根据 `plus`、`max` 字符串自行推断权限。
...
主动打招呼默认不限制扫描批数，不得再使用固定三批作为结束条件。每批完成后调用平台的统一候选人列表推进能力，持续加载下一批；只有达到岗位本次打招呼上限、平台确认没有更多候选人、用户手动停止、云端状态不允许继续或统一错误策略触发时才结束。若云端以后明确下发 `max_batches` 或 `scan_rounds`，才按该值限制批数。结束前必须写一条用户可见的原因说明。
...
AutoReplyFlow
  1. 启动或复用浏览器
  2. 打开对应 Profile
  3. 根据当前页面域名获取平台 Runtime
  4. 每轮检查云端登录态、会员和岗位自动回复开关
  5. 读取消息悬浮入口的未读数量
  6. 打开联系人侧边栏并形成未读会话快照
  7. 打开会话并校验候选人、平台账号和页面沟通岗位
  8. 首次同步历史或差量同步新消息
  9. 检查正式简历和附件；缺失时索要、下载、清洗并同步
  10. 运行本地 AI tool_calls 循环并维护候选人确认项
  11. 发送前再次读取最新消息和当前会话身份
  12. 调用平台消息发送能力并回读确认
  13. 同步 AI 总记录、工具结果和实际发送结果
  14. 单轮最多处理3个会话，再释放浏览器给外层调度器
  15. 完成当前会话后响应停止，或按间隔进入下一轮
```

自动回复不得复用主动打招呼的完整主流程，只复用明确的公共能力。主动打招呼每处理一位候选人前由外层调度器读取自动回复开关；两个主流程不能互相调用。

自动回复详细决定和阶段进度见 `docs/auto-reply-decisions.md` 与 `docs/auto-reply-roadmap.md`。
...
- Go、Worker、AI 分析和自动回复工具调用摘要统一按任务编号写入全局日志，全局只保留最近 5000 条。
