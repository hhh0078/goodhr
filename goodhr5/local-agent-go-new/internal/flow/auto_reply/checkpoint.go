...
func (f *Flow) processCheckpoint(ctx context.Context, prepared shared.PreparedTask, runtime model.Runtime, replyRuntime model.AutoReplyRuntime, positions []cloud.AutoReplyPositionSnapshot, limit int, stats *shared.Stats, errorPolicy *shared.ConsecutiveErrorPolicy) (int, error) {
	conversations, err := runtime.ScanUnreadConversations(ctx, f.Browser, prepared.Platform)
	if err != nil {
		return 0, fmt.Errorf("读取未读候选人列表失败：%w", err)
	}
