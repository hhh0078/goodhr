...
- 启动本地 HTTP 服务，默认优先使用 `127.0.0.1:95271`，被占用时尝试 `95272-95279`。
...
- 已接入本地 OCR 状态和识别接口，任务 OCR 模式可调用 RapidOCR-json 类常驻组件。
- 已接入本地 AI 打招呼评分。
...
- 已接入 Node Worker 启动等待、调用失败自动重启、停止任务主动关闭浏览器和进程树清理。
- 已接入截图默认本地保存和截图记录落库。
