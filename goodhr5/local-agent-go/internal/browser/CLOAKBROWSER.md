...
招聘平台要求稳定登录，因此 GoodHR 应继续为同一个本地用户复用固定资料目录，不能在每次任务中临时生成新资料目录。
...
- 在替换 Node Worker 前，必须对 BOSS、猎聘、智联等真实站点分别进行登录稳定性、风控、指纹和长时间任务测试；
...
- GoodHR 本地程序版本 `5.1.9` 与 CloakBrowser/npm/Chromium 版本是三套不同版本，不能混用。
