...
func TestScrollCandidateDetailTargetsResumeDialog(t *testing.T) {
	...
	if exec.payload["skip_mouse_move"] != true {
		t.Fatalf("智联详情滚动应直接使用当前鼠标位置，不再移动鼠标：%+v", exec.payload)
	}
	if _, exists := exec.payload["element"]; exists {
		t.Fatalf("智联直接滚动不应再查找详情选择器：%+v", exec.payload)
	}
}
