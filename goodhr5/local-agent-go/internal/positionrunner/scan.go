...
// ctx 为请求上下文，position 为岗位运行记录，platformConfig 为云端平台配置。
func (r *Runner) scanOnce(ctx context.Context, position localdb.Position, platformConfig cloudapi.PlatformConfig, options StartOptions) (map[string]any, error) {
	if r.worker == nil {
...
scanLoop:
	for emptyLoads < emptyLimit {
		...
		if err := r.ensureCloudSessionActive(ctx, position, options); err != nil {
			return nil, err
		}
		if len(queue) == 0 {
			...
			if skipPositionSelection {
				...
				if forcePositionSelection {
					...
					if strings.Contains(normalizePositionName(currentName), normalizePositionName(positionName)) {
						...
						confirmedName, err := r.waitCurrentPositionName(ctx, position.ID, platformRuntime, exec, platformConfig)
						if err != nil {
		...
		if len(filtered) > 0 {
			...
			for nextIndex < len(filtered) {
				...
				// 6. 非 AI 主模式下，如果岗位运行要求看详情，也按配置读取详情。
				if !needsAI && shouldFetchDetail(position) && canContinueCandidate(stringFromMap(candidate, "status")) {
				...
				// 7. 第二次详情分析：详情 AI 已经一次性评分时跳过；否则按岗位运行模式做最终判断。
				if _, supportsDetailScrolling := platformRuntime.(platformcore.DetailAnalysisScroller); detailSession != nil && !supportsDetailScrolling {
					if closeErr := detailSession.Close(context.WithoutCancel(candidateCtx)); closeErr != nil {
						candidateCancel()
						return nil, fmt.Errorf("候选人详情无法关闭，岗位运行已自动停止：%w", closeErr)
					}
					detailSession = nil
				}
				...
				if detailSession != nil && shouldFinalizeWithAI && positionMode(position) != "keyword" {
					stopDetailScrolling = r.startCandidateDetailScrolling(candidateCtx, position.ID, platformRuntime, exec, platformConfig, candidate)
				}
				if shouldFinalizeWithAI {
					...
					if err != nil {
						if shouldStopPositionImmediately(err) {
							candidateCancel()
							return nil, fmt.Errorf("AI最终评分持续不可用，岗位运行已自动停止：%w", err)
						}
						candidate["status"] = "failed"
						candidate["error"] = err.Error()
						batchResult.Failed++
...
// ctx 为岗位运行上下文，position 为本地岗位运行，options 为启动参数。
func (r *Runner) ensureCloudSessionActive(ctx context.Context, position localdb.Position, options StartOptions) error {
...
// ctx 为请求上下文，positionID 为岗位运行 ID，platformRuntime 为平台执行器，exec 为 Worker 执行器，platformConfig 为平台配置，options 为岗位运行启动参数。
func (r *Runner) scrollForMoreCandidates(ctx context.Context, positionID string, platformRuntime platformcore.Runtime, exec platformExecutor, platformConfig cloudapi.PlatformConfig, options StartOptions) error {
	if err := ctx.Err(); err != nil {
		return err
	}
...
// ctx 为候选人处理上下文，positionID 为岗位运行 ID，platformRuntime 为平台运行时，exec 为 Worker 执行器，platformConfig 为平台配置，candidate 为候选人。
func (r *Runner) ensureCandidateVisibleBeforeDecision(ctx context.Context, positionID string, platformRuntime platformcore.Runtime, exec platformExecutor, platformConfig cloudapi.PlatformConfig, candidate platformcore.Candidate) {
...
// ctx 为请求上下文，position 为岗位运行记录，platformConfig 为云端平台配置。
func (r *Runner) ensurePositionPageReady(ctx context.Context, position localdb.Position, platformRuntime platformcore.Runtime, exec platformExecutor, platformConfig cloudapi.PlatformConfig) error {
	if err := ctx.Err(); err != nil {
		return err
	}
	if err := r.waitPositionEntryPage(ctx, position.ID, platformRuntime, exec, platformConfig); err != nil {
...
// positionID 为岗位运行 ID，platformRuntime 为平台实现，exec 为浏览器执行器，platformConfig 为云端平台配置。
func (r *Runner) prepareEntryPage(ctx context.Context, positionID string, platformRuntime platformcore.Runtime, exec platformExecutor, platformConfig cloudapi.PlatformConfig) {
	if err := ctx.Err(); err != nil {
		return
	}
...
// ctx 为请求上下文，positionID 为岗位运行 ID，platformConfig 为平台配置。
func (r *Runner) waitPositionEntryPage(ctx context.Context, positionID string, platformRuntime platformcore.Runtime, exec platformExecutor, platformConfig cloudapi.PlatformConfig) error {
...
// ctx 为请求上下文，positionID 为岗位运行 ID，platformConfig 为平台配置。
func (r *Runner) waitCurrentPositionName(ctx context.Context, positionID string, platformRuntime platformcore.Runtime, exec platformExecutor, platformConfig cloudapi.PlatformConfig) (string, error) {
...
// browserDownloadDir 返回岗位运行运行时使用的下载目录。
...
// platformEntryPage 读取平台岗位运行入口页配置。
// platformConfig 为云端平台配置。
...
// positionPositionName 返回岗位运行岗位名称。
// position 为岗位运行记录。
func positionPositionName(position localdb.Position) string {
	return stringFromMap(position.PositionSnapshot, "name")
}

// normalizePositionName 规范化岗位名称用于比较。
// value 为原始岗位名称。
