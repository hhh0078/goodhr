...
- 保持现有业务逻辑不变：平台配置从云端读取，数据尽量存本地，开始岗位运行前仍校验会员。
...
- 启动本地 HTTP 服务，默认优先使用 `127.0.0.1:55271`，被占用时尝试 `55272-55279`。
...
- 迁移本地岗位运行、岗位、候选人、下载记录、截图记录、AI 配置。
- 本地岗位运行启动时从云端公开接口读取平台配置。
- 岗位运行开始前校验会员。
- 岗位运行日志全部落本地。
...
- 已接入本地 SQLite 数据库。
- 已完成本地岗位运行、日志、候选人、岗位模板、AI 配置、通用设置、下载记录和截图记录的基础接口。
- 已完成云端平台配置公开读取和会员状态校验客户端。
- 已接入本地岗位运行运行器骨架，开始岗位运行时会先校验会员并读取云端平台配置。
- 已接入 Boss 候选人第一轮扫描和本地候选人保存。
- 已拆分详情读取模式：DOM、OCR、AI 三种方式互相独立，用户选择哪种就执行哪种。
- 已接入本地 OCR 状态和识别接口，岗位运行 OCR 模式可调用 RapidOCR-json 类常驻组件。
- 已接入本地 AI 打招呼评分。
- 已接入受控 Boss 打招呼动作，必须显式传入 `enable_greet=true` 才会点击。
- 已接入岗位运行停止取消、打招呼前随机等待和失败重试。
- 已接入按云端平台配置滚动候选人列表容器，避免只滚动页面导致多轮扫描停住。
- 已接入岗位运行后台异步运行和运行状态查询。
- 已接入岗位运行进度、最近日志返回和前端状态查询服务方法。
- 已接入扫描轮数、每轮提取数量、滚动距离等岗位运行运行参数。
- 已接入前端岗位运行列表进度轮询和进度条展示。
- 已接入前端本地岗位运行运行参数表单。
- 已接入本地浏览器 Profile 元数据接口，平台账号可按账号隔离浏览器目录。
- 已接入下载目录默认值、本地设置覆盖、下载记录去重和文件名后缀修复。
- 已接入当前页面 URL、本地 AI 聊天、岗位默认提示词、岗位要求优化、规则状态等前端本地模式接口。
- 已接入 Node Worker 启动等待、调用失败自动重启、停止岗位运行主动关闭浏览器和进程树清理。
- 已接入截图默认本地保存和截图记录落库。
- 已接入本地控制台前端包状态和更新接口，支持从 OSS manifest 下载 zip 包到本地数据目录。
- 已提供 Node Worker 发布包脚本和 Windows 安装器骨架，正式 OSS 上传流程仍待最后统一处理。
