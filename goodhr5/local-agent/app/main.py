...
async def _show_click_marker(page, matched_target: str, probe: dict, box: dict | None, status: str, hold_ms: int = 1800) -> None:
    """
    在页面上显示点击调试标记。
...
async def _safe_random_click(page, locator, matched_target: str) -> None:
    ...
    await _show_click_marker(page, matched_target, latest_probe, latest_box, "准备点击")
    if not latest_probe.get("ok"):
        await _show_click_marker(page, matched_target, latest_probe, latest_box, "点击失败-状态异常", 5000)
        raise HTTPException(400, f"点击目标元素状态检查失败: {matched_target}, probe={latest_probe}")
    if not latest_probe.get("connected", False):
        await _show_click_marker(page, matched_target, latest_probe, latest_box, "点击失败-节点失效", 5000)
    ...
    if not latest_probe.get("inViewport", False):
        ...
        await _show_click_marker(page, matched_target, latest_probe, latest_box, "目标在视口外，准备滚动", 2600)
        if not await scroll_locator_into_view(locator, matched_target):
            latest_probe = await _probe_click_state(locator)
            latest_box = await locator.bounding_box()
            await _show_click_marker(page, matched_target, latest_probe, latest_box, "点击失败-滚动后仍在视口外", 6000)
        ...
        await _show_click_marker(page, matched_target, latest_probe, latest_box, "滚动后准备点击")
        if not latest_probe.get("inViewport", False):
            await _show_click_marker(page, matched_target, latest_probe, latest_box, "点击失败-滚动后仍在视口外", 6000)
            raise HTTPException(400, f"点击目标元素不在视口内: {matched_target}, probe={latest_probe}")
    if not latest_probe.get("centerHit", False):
        await _show_click_marker(page, matched_target, latest_probe, latest_box, "点击失败-中心被遮挡", 6000)
    ...
    if not latest_box or latest_box.get("width", 0) <= 0 or latest_box.get("height", 0) <= 0:
        await _show_click_marker(page, matched_target, latest_probe, latest_box, "点击失败-位置无效", 6000)
    ...
    if not await click_box_random_point(page, latest_box, matched_target):
        await _show_click_marker(page, matched_target, latest_probe, latest_box, "点击失败-随机点失败", 6000)
