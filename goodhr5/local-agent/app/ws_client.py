...
def _payload_summary(payload: Any) -> str:
    ...
    if isinstance(body.get("ai_vision"), dict):
        parts.append("ai_vision=已传")
    ...
    async def _execute_local_post(self, payload: dict) -> dict:
        ...
        if path == "/api/v1/page/extract-text":
            ...
            for index, element_raw in enumerate(raw_elements):
                ...
                text = await self._extract_text_from_locator(page, locator, mode, delay_before, task_id_for_screenshot, f"element_{index}", body.get("ai_vision"))
        ...
        if path == "/api/v1/page/list-click-by-index":
            ...
            if container_box is None: raise ValueError("列表容器定位失败")
