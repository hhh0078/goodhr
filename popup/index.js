...
// API配置
const API_BASE = window.GOODHR_CONFIG
  ? window.GOODHR_CONFIG.API_BASE
  : "https://goodhr.58it.cn";
...
async function saveSettings() {
  try {
    ...
    // 更新serverData中的设置
    serverData.isAndMode =
      document.getElementById("keywords-and-mode")?.checked || false;
    serverData.matchLimit =
      parseInt(document.getElementById("match-limit")?.value) || 200;
    serverData.enableSound =
      document.getElementById("enable-sound")?.checked || true;
    serverData.scrollDelayMin =
      parseInt(document.getElementById("delay-min")?.value) || 3;
    serverData.scrollDelayMax =
      parseInt(document.getElementById("delay-max")?.value) || 5;
    serverData.clickFrequency =
      parseInt(document.getElementById("click-frequency")?.value) || 7;

    // 保存公司信息
    if (!serverData.companyInfo) {
      serverData.companyInfo = {};
    }
    serverData.companyInfo.content =
    ...
    serverData.jobInfo.extraInfo =
      document.getElementById("job-extra-info")?.value || "";
    ...
    serverData.runModeConfig.greetingEnabled =
      document.getElementById("greeting-checkbox")?.checked !== false; // 默认为true
    serverData.runModeConfig.communicationEnabled =
      document.getElementById("communication-checkbox")?.checked !== false; // 默认为true

    // 索要手机号
    serverData.communicationConfig.collectPhone =
      document.querySelector(".collect-phone")?.checked || false;
    // 索要微信号
    serverData.communicationConfig.collectWechat =
      document.querySelector(".collect-wechat")?.checked || false;
    // 索要简历
    serverData.communicationConfig.collectResume =
      document.querySelector(".collect-resume")?.checked || false;
...
function addExcludeKeyword() {
  ...
  if (
    keyword &&
    !serverData.currentPosition.excludeKeywords.includes(keyword)
  ) {
...
function renderExcludeKeywords() {
  ...
  const currentExcludeKeywords = serverData.currentPosition
    ? serverData.currentPosition.excludeKeywords
    : [];
...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    // 绑定刷新指纹按钮事件
    const refreshFingerprintBtn = document.getElementById(
      "refresh-fingerprint",
    );
    if (refreshFingerprintBtn) {
      refreshFingerprintBtn.addEventListener("click", async () => {
        try {
          // 显示加载状态
          document.getElementById("fingerprint-display").textContent =
            "刷新中...";
    ...
    // 设置版本号 - 使用配置文件中的版本
    const version = window.GOODHR_CONFIG
    ...
    if (aiClickPrompt) {
      ...
      aiClickPrompt.addEventListener("input", () => {
        ...
        aiPromptSaveTimeout = setTimeout(async () => {
          try {
            // 更新serverData中的提示语
            serverData.ai_config.clickPrompt = document
              .getElementById("ai-click-prompt")
              .value.trim();
    ...
    const collectWechatCheckboxes =
      document.querySelectorAll(".collect-wechat");
    const collectResumeCheckboxes =
      document.querySelectorAll(".collect-resume");
    ...
    // AI优化岗位描述按钮事件
    const aiOptimizeBtn = document.getElementById(
    ...
    document
      .getElementById("add-position")
      ?.addEventListener("click", addPosition);

    // 绑定AI版本职位相关事件
    document
      .getElementById("ai-position-input")
      ?.addEventListener("keypress", (e) => {
        if (e.key === "Enter") {
          addPosition();
        }
    ...
    document
      .getElementById("ai-add-position")
      ?.addEventListener("click", addPosition);
...
async function startAutoScroll() {
  ...
  try {
    ...
    if (currentTab === "ai") {
      ...
      addLog(
        `随机延迟时间ai: ${delayMinInput.value}-${delayMaxInput.value}秒`,
        "info",
      );
      ...
      // 检查是否有关键词
      if (
        !serverData.currentPosition.keywords.length &&
        !serverData.currentPosition.excludeKeywords.length
      ) {
        if (
          !confirm(
            "当前岗位没有设置任何关键词，将会给所有候选人打招呼，是否继续？",
          )
        ) {
          return;
        }
      ...
      addLog(
        `随机延迟时间免费: ${delayMinInput.value}-${delayMaxInput.value}秒`,
        "info",
      );
...
function renderKeywords() {
  ...
  // 为每个关键词创建元素
  const currentKeywords = serverData.currentPosition
    ? serverData.currentPosition.keywords
    : [];
...
chrome.runtime.onMessage.addListener(async (message, sender, sendResponse) => {
  ...
  if (message.type === "MATCH_SUCCESS") {
    const { name, age, education, university, extraInfo, clicked } =
      message.data;
    matchCount++;
    ...
    if (serverData.enableSound) {
      const audio = new Audio(
        chrome.runtime.getURL("sounds/notification2.mp3"),
      );
...
function addPosition() {
  ...
  if (
    positionName &&
    !serverData.positions.find((p) => p.name === positionName)
  ) {
    const newPosition = {
      name: positionName,
      keywords: [],
...
function removePosition(positionName) {
  if (
    confirm(
      `确定要删除职位"${positionName}"吗？\n删除后该职位的所有关键词都将被删除。`,
    )
  ) {
    serverData.positions = serverData.positions.filter(
...
function selectPosition(positionName) {
  serverData.currentPosition = serverData.positions.find(
    (p) => p.name === positionName,
  );
...
function updateJobDescription() {
  ...
  const jobDescriptionGroup = document.getElementById(
    "ai-job-description-group",
  );

  // 只有在AI模式且选中了岗位时才显示岗位说明
  if (
    currentTab === "ai" &&
    serverData.currentPosition &&
    jobDescriptionGroup
  ) {
...
function renderRankingList(data) {
  ...
  if (!Array.isArray(data) || data.length === 0) {
    container.innerHTML =
      '<div class="ranking-item" style="text-align: center; color: #666;">暂无打赏数据</div>';
    return;
  }
...
// 检查AI功能是否可用 - 现在总是返回true，移除了到期时间限制
async function checkAIAvailability() {
  // 移除到期时间检测，AI功能总是可用
  console.log("AI功能可用，移除了到期时间限制");
  return true;
}
...
async function initializeAIExpireTime() {
  try {
    ...
    if (
      trialStatus.used &&
      trialStatus.associatedPhone &&
      trialStatus.associatedPhone !== boundPhone
    ) {
      addLog(
        `此设备已绑定手机号 ${trialStatus.associatedPhone}，无法为新手机号设置试用期`,
        "warning",
      );
      return;
    }
  ...
  if (boundPhone) {
    try {
      ...
      if (result.success) {
        ...
        if (
          result.message &&
          result.message.includes("此手机号已绑定其他设备")
        ) {
...
async function bindPhone(phone) {
  try {
    ...
    if (
      checkResult.device_used &&
      checkResult.associated_phone &&
      checkResult.associated_phone !== phone
    ) {
      throw new Error(
        `此设备已绑定手机号 ${checkResult.associated_phone}，无法绑定新手机号`,
      );
    }
    ...
    if (
      checkResult.error &&
      checkResult.error.includes("此手机号已绑定其他设备")
    ) {
    ...
    try {
      ...
      addLog(
        `AI平台账号创建成功，初始余额: ¥${createResponse.data.balance}`,
        "success",
      );
    } catch (error) {
      addLog(`AI平台账号创建失败: ${error.message}`, "warning");
    }
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (settings.currentPosition) {
    ...
    const targetPosition = serverData.positions.find(
      (p) => p.name === settings.currentPosition,
    );
    if (targetPosition) {
      selectPosition(settings.currentPosition);
    }
...
async function loadAIConfig() {
  try {
    ...
    if (serverData.ai_config && serverData.ai_config.model) {
...
async function saveAIConfig() {
  try {
    ...
    // 验证提示语是否包含必要的标记符
    if (
      !serverData.ai_config.clickPrompt.includes("${候选人信息}") ||
      !serverData.ai_config.clickPrompt.includes("${岗位信息}")
    ) {
      throw new Error(
    ...
    // 验证提示语是否要求返回JSON格式
    if (
      !serverData.ai_config.clickPrompt.includes("JSON") &&
      !serverData.ai_config.clickPrompt.includes("json")
    ) {
      console.warn(
...
async function checkAIConnection() {
  ...
  if (!boundPhone) {
    ...
    statusText.textContent = "未绑定手机号";
    hideBalanceDisplay(); // 未绑定时隐藏余额显示
    return;
  }

  // 检查AI功能是否可用 - 移除了到期时间检测
  console.log("检查AI功能是否可用...");
  const isAIAvailable = await checkAIAvailability();
  console.log("AI功能是否可用:", isAIAvailable);
  if (!isAIAvailable) {
    statusIndicator.className = "ai-status-indicator disconnected";
    statusText.textContent = "AI功能不可用";
    hideBalanceDisplay(); // 不可用时隐藏余额显示
    console.log("AI功能不可用");
    return;
  }
  ...
  if (boundPhone) {
    ...
    statusText.innerHTML =
      '<span >* 获取秘钥请访问：<a href="https://goodhr.58it.cn/describe.html" target="_blank">AI配置教程</a></span>';
...
function updatePriceDisplay() {
  ...
  for (const element of priceElements) {
    if (
      element.textContent.includes("价格") &&
      element.textContent.includes("元")
    ) {
      // 更新价格文本
...
function createAdElement(adData, position) {
  ...
  if (adData.image) {
    content =
      `<img src="${adData.image}" style="max-width: 100%; height: auto; margin-bottom: 4px;">` +
      content;
  }
...
function displayAds() {
  ...
  // 获取滚动容器
  const scrollContainer = freeTab.querySelector(
  ...
  if (adConfig.ads.middle) {
    ...
    if (middleAd) {
      ...
      if (filterGroups.length >= 3) {
        filterGroups[2].parentNode.insertBefore(
          middleAd,
          filterGroups[2].nextSibling,
        );
      } else if (filterGroups.length > 0) {
        filterGroups[filterGroups.length - 1].parentNode.insertBefore(
          middleAd,
          filterGroups[filterGroups.length - 1].nextSibling,
        );
      }
    }
  }
...
async function updateServerData() {
  ...
  const response = await fetch(
    `${API_BASE}/updatejson.php?phone=${boundPhone}`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
function updateOtherSettingsUI() {
  ...
  collectPhoneCheckboxes.forEach((checkbox) => {
    if (
      checkbox &&
      serverData.communicationConfig &&
      serverData.communicationConfig.collectPhone !== undefined
    ) {
  ...
  collectWechatCheckboxes.forEach((checkbox) => {
    if (
      checkbox &&
      serverData.communicationConfig &&
      serverData.communicationConfig.collectWechat !== undefined
    ) {
  ...
  collectResumeCheckboxes.forEach((checkbox) => {
    if (
      checkbox &&
      serverData.communicationConfig &&
      serverData.communicationConfig.collectResume !== undefined
    ) {
  ...
  if (
    companyInfoElement &&
    serverData.companyInfo &&
    serverData.companyInfo.content !== undefined
  ) {
  ...
  if (
    jobExtraInfoElement &&
    serverData.jobInfo &&
    serverData.jobInfo.extraInfo !== undefined
  ) {
  ...
  communicationFields.forEach((field) => {
    ...
    if (
      element &&
      serverData.communicationConfig &&
      serverData.communicationConfig[field.replace(/-/g, "")] !== undefined
    ) {
      if (element.type === "checkbox") {
        element.checked =
          serverData.communicationConfig[field.replace(/-/g, "")];
      } else {
  ...
  runModeFields.forEach((field) => {
    ...
    if (
      element &&
      serverData.runModeConfig &&
      serverData.runModeConfig[field.replace("-checkbox", "")] !== undefined
    ) {
      element.checked =
        serverData.runModeConfig[field.replace("-checkbox", "")];
    }
  });
}
